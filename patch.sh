...
+    const outputChannel = vscode.window.createOutputChannel('K8s Debug');
...
+        outputChannel.appendLine('Namespaces:');
+        for (const ns of res.items) {
+            outputChannel.appendLine(`- ${ns.metadata?.name}`);
         }
...
-        return resource;
+    } catch (err) {
+        outputChannel.appendLine(`Failed to list namespaces: ${err}`);
     }
 }
